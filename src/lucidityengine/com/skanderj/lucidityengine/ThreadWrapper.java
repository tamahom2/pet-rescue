	...
	protected final void stop() {
		if (this.isRunning) {
			...
			try {
				...
				Logger.log(this.getClass(), LogLevel.DEBUG, "An interrupted exception has been raised while stopping a thread and can be safely ignored.");
			}
		} else {
			return;
		}
		...
		while (this.isRunning) {
			if (this.isPaused) {
				try {
					...
					Logger.log(this.getClass(), LogLevel.DEBUG, "An interrupted exception has been raised while a paused thread sleeps and can be safely ignored.");
				}
			} else {
				this.cycle();
			}
