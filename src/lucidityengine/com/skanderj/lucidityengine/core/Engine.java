...
import com.skanderj.lucidityengine.graphics.components.Components;
...
public final class Engine {
	...
	private static final Map<String, ApplicationObject> registry = new HashMap<>();
	...
	public static void registerScene(final String identifier, final Scene scene) {
		...
		Logger.log(Engine.class, LogLevel.DEBUG, "Registering scene %s: %s", identifier, scene);
	}
	 ...
	 */
	public static void switchToScene(final String newSceneIdentifier) {
		...
		if (Engine.currentScene == null) { // If there's no current scene, we can go straight to the in animation of the
			...
			Engine.currentScene.enter();
			Components.giveFocus("void");
...
//					}
					...
					TaskScheduler.scheduleTask("transition", new Task(Utilities.framesToMS(Engine.currentScene.inTransition().durationInFrames(), Engine.currentScene.application().getTargetUpdatesPerSecond()), Task.NO_REPEATS, TimeUnit.MILLISECONDS) {
						@Override
						...
						public void execute() {
							...
							Engine.currentScene.removeComponent("current-scene-in-transition");
							if (Engine.currentScene.afterInTransition() != null) {
								Engine.currentScene.afterInTransition().execute(Engine.currentScene);
							}
						}
					});
...
//				}
				TaskScheduler.scheduleTask("transition", new Task(Utilities.framesToMS(Engine.currentScene.outTransition().durationInFrames(), Engine.currentScene.application().getTargetUpdatesPerSecond()), Task.NO_REPEATS, TimeUnit.MILLISECONDS) {
					@Override
					public TaskType type() {
						return TaskType.FIXED_DELAY;
					}
					...
					public void execute() {
						Engine.currentScene.removeComponent("current-scene-out-transition");
						if (Engine.currentScene.afterOutTransition() != null) {
							Engine.currentScene.afterOutTransition().execute(Engine.currentScene);
						}
...
//					}
	 ...
	 */
	public static void setObject(final String identifier, final ApplicationObject object) {
		String name;
		...
		Logger.log(Engine.class, LogLevel.INFO, "Added to registry: '%s' <- <class : %s>", identifier, object.getClass().getSimpleName().equals("") ? object.getClass().getEnclosingClass().getSimpleName() + "#" + name : object.getClass().getSimpleName());
	}
	 ...
	 */
	public static void unsetObject(final String identifier) {
	 ...
	 */
	public static ApplicationObject getObject(final String identifier) {
	 ...
	 */
	protected static void updateObjects() {
		final Set<String> identifiersToUpdate = Engine.currentScene.sceneObjectsIdentifiers();
		final List<ApplicationObject> objects = new ArrayList<>();
	 ...
	 */
	protected static void renderObjects() {
		final Set<String> identifiersToUpdate = Engine.currentScene.sceneObjectsIdentifiers();
		final List<ApplicationObject> objects = new ArrayList<>();
		for (final String identifier : Engine.registry.keySet()) {
			if (identifiersToUpdate.contains(identifier)) {
