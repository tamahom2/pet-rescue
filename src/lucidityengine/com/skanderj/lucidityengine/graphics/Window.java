...
public final class Window {
	...
	public Window(final Application application, final String title, final int width, final int height, final boolean fullscreen) {
		this(application, title, width, height, fullscreen, 2);
	}

	public Window(final Application application, final String title, final int width, final int height, final boolean fullscreen, final int buffers) {
		this(application, title, width, height, fullscreen, buffers, 0);
	}

	public Window(final Application application, final String title, final int width, final int height, final boolean fullscreen, final int buffers, final int deviceId) {
	...
	public Window(final Application application, final WindowConfiguration configuration) {
		...
		// Frame
		{
			...
			if (configuration.getIconImagePath() != null) {
				try {
					this.frame.setIconImage(ImageIO.read(new File(configuration.getIconImagePath())));
				} catch (IOException ioException) {
					Logger.log(Window.class, LogLevel.ERROR, "Invalid path for window's icon image! (%s)", ioException.getMessage());
				}
			}
		}
		...
		if ((configuration.deviceId < 0) || (configuration.deviceId >= devices.length)) {
			Logger.log(this.getClass(), LogLevel.SEVERE, "Invalid device ID %d, falling back to default", configuration.deviceId);
		...
		if (configuration.fullscreen) {
			if (device.isFullScreenSupported()) {
				...
				Logger.log(this.getClass(), LogLevel.SEVERE, "Can't enter fullscreen or change display mode on this device!");
	...
	public void hide(final boolean dispose) {
	...
	public void registerInputDevice(final InputDevice device) {
		switch (device.type()) {
		case KEYBOARD:
