...
public final class Fonts {
	...
	public static boolean load(final String identifier, final String path) {
		...
		try {
			...
			Logger.log(Fonts.class, LogLevel.INFO, "Font loaded: '%s' <- %s (%d ms)", identifier, fontFile.getPath(), endTime - startTime);
			return true;
		} catch (FontFormatException | IOException exception) {
			Logger.log(Fonts.class, LogLevel.SEVERE, "An exception occurred while loading font from %s: %s", path, exception.getMessage());
			return false;
		}
	}
