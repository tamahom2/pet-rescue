...
/**
 *	Représente une Demo de l'interface graphique
...
public final class LucidityEngineDemo extends Application {
	...
	public static final int WINDOW_WIDTH = 1200, WINDOW_HEIGHT = (int) ((LucidityEngineDemo.WINDOW_WIDTH / 16.0) * 9), BUFFERS = 2;
	public static final double TARGET_REFRESH_RATE = 144.0;
	public static final boolean FULLSCREEN = false;

	public LucidityEngineDemo() {
		super(LucidityEngineDemo.DEMO_IDENTIFIER, LucidityEngineDemo.TARGET_REFRESH_RATE, LucidityEngineDemo.TARGET_REFRESH_RATE, Keyboard.AZERTY);
	}
	...
	protected void registerApplicationObjects() {
		Engine.setObject("space-bg", new ImageBackground(this, 0, 0, this.windowConfiguration().getWidth(), this.windowConfiguration().getHeight(), Images.get("space-bg")));
		Engine.setObject("title-label-flicker", new FlickeringLabel(this, 0, 0, this.windowConfiguration().getWidth(), this.windowConfiguration().getHeight() / 4, "Lucidity Engine", new OnScreenTextProperties(Fonts.get("never", 72), Utilities.buildAgainst(Color.MAGENTA.darker(), 35), Color.GRAY, OnScreenTextPosition.CENTERED)));
		Engine.setObject("subtitle-label-flicker", new FlickeringLabel(this, 0, 0, this.windowConfiguration().getWidth(), this.windowConfiguration().getHeight() / 2, "[DEMO]", new OnScreenTextProperties(Fonts.get("never", 48), Utilities.buildAgainst(Color.PINK.darker(), 35), Color.GRAY, OnScreenTextPosition.CENTERED)));
	}
	...
	protected void registerScenes() {
		...
		((Scene) Engine.getObject("main-menu")).setOutTransition(new FadeOutTransition(this, 144 * 3, Color.BLACK));
	...
	protected void registerBinds() {
		Binds.registerBind("main-menu", Utilities.createArray(Keyboard.KEY_SPACE), Utilities.createArray(Keyboard.KeyState.DOWN_IN_CURRENT_FRAME), object -> Engine.switchToScene("main-menu-2"));
		Binds.registerBind("main-menu-2", Utilities.createArray(Keyboard.KEY_ESCAPE), Utilities.createArray(Keyboard.KeyState.DOWN_IN_CURRENT_FRAME), object -> Engine.switchToScene("main-menu"));
	}
	...
	protected WindowConfiguration windowConfiguration() {
		return new WindowConfiguration(LucidityEngineDemo.WINDOW_TITLE, LucidityEngineDemo.WINDOW_WIDTH, LucidityEngineDemo.WINDOW_HEIGHT, LucidityEngineDemo.FULLSCREEN, LucidityEngineDemo.BUFFERS);
	}
}
