...
public class FlickeringLabel extends ApplicationObject {
	...
	public FlickeringLabel(final Application application, final int x0, final int y0, final int width, final int height, final String content, final OnScreenTextProperties onScreenTextProperties) {
		super(application);
		this.x0 = x0;
		this.y0 = y0;
		this.width = width;
	...
	@Override
	public void render() {
		new OnScreenText(this.content, this.properties.build(this.flickerColor)).draw(this.application.screen(), this.x0 + this.xFlicker, this.y0 + this.yFlicker, this.width, this.height);
	}
	...
	public String toString() {
		return String.format("FlickeringText (params: x0=%d, y0=%d, content=%s, properties=%s", this.x0, this.y0, this.content, this.properties);
	}
}
